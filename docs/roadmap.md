...
- [x] Rediger/slet egne steder — pencil/papirkurv-knapper på et sted med `isUserSpot`, kun der. Nyt sted bliver automatisk det aktive med det samme (var før kun et krav i UI'et, nu i selve `addUserSpot` i AppContext, så det gælder uanset hvor kaldet kommer fra). `spots_update` fik en manglende WITH CHECK rettet i samme omgang (migration 0012) — uden den kunne en håndlavet PATCH i teorien gøre et privat sted offentligt ved at ændre `source` til 'system'
- [x] Rettet: PWA'en var altid én genindlæsning bagud efter en udrulning — `registerType: 'autoUpdate'` installerer en ny service worker, men uden en `controllerchange`-lytter tog den først effekt på den ANDEN genindlæsning. Så en pushet rettelse virkede ikke, selvom den reelt lå live — det første "prøv at genindlæse" hentede bare den nye service worker, ikke den nye kode. Se `src/main.tsx`
- [x] Rettet: et eget sted forsvandt igen fra stedvælgeren efter en genindlæsning, selvom det stadig lå korrekt i databasen. Root cause: `relations` (fastgjort/fulgt/skjult) lå kun i React-state, aldrig gemt — et nyt sted sættes til 'followed' ved oprettelsen, men det gik tabt ved næste genindlæsning, uanset spot eller system. Rettet på to niveauer: `relations` gemmes nu i localStorage (samme mønster som `activeSpotId`), OG `rankSpots` giver et `isUserSpot`-sted en standardrelation på 'followed', hvis ingen er gemt — sidstnævnte holder også på en anden enhed eller efter ryddet storage, hvor localStorage ikke følger med. To nye tests i ranking.test.ts
